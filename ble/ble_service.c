...
BT_GATT_SERVICE_DEFINE(dwm1001_svc,
    ...
    BT_GATT_CHARACTERISTIC(BT_UUID_DWM1001_NOTIFY, BT_GATT_CHRC_READ | BT_GATT_CHRC_NOTIFY,
        BT_GATT_PERM_READ, 
        NULL, NULL, NULL),
...
static uint8_t fs_gatt_start[1];
int dwm1001_notify(uint8_t * data, uint32_t len)
{
    ...
    if (is_connected()) {
      if (true){
        ...
        int rc = bt_gatt_notify(NULL, &dwm1001_svc.attrs[1], data, sizeof(len));
        printk ("%d", rc);
        return rc;
      }
      return 0;
